                   ...
                   int *primers_in_pool, PNODE ***primer_pool)
{
    ...
    int ppairs_count = 0;
    int final_pool_number = 0;

    char header[1024];
    fgets( header, 1024, primer_file );
    char *fields;
    fields = strtok( header, "\t");
    check((strcmp(fields, "Name")==0), "%s does not start with header of primer file", filename);
    ...
    while (!feof(primer_file))
    {
        ...
        char primer_name[1024];
        int this_primer_num, this_pool_num;
        this_primer_num = this_pool_num = 0;

        int read_line =
        fscanf (primer_file,
                "%s\t%d\t%d\t%s\t%lg\t%lg\t%s\t%lg\t%lg\t%d\t%d\t%d\t%d\t%d\t%d\t%lg\t%lg\t%s\n",
                primer_name,
                &this_pool_num,
                &this_primer_num,
                ...
                &temp_ppair->tm,
                temp_ppair->sequence);
        check((read_line == 18 ), "Error processing file: %s", filename);
        check((ppairs_count < max_primer_pairs), "Out of Memory. Increase primer_pool size.");
        check((this_primer_num < max_primers_in_pool), "Too many primer pairs in pool %d", this_pool_num);

        primer_pool[this_pool_num][this_primer_num] = temp_ppair;
        debug("assigned primer pair id '%d' to pool number '%d'", this_primer_num, this_pool_num);

        primers_in_pool[this_pool_num]++;
        ppairs_count++;
        debug("\n\n\tpool = %d, pool primer pairs count = %d, total primer pairs count  = %d\n",
              this_pool_num, primers_in_pool[this_pool_num], ppairs_count);
        final_pool_number = this_pool_num;
    }

    debug("returning this_pool_num: %d", final_pool_number);
    return final_pool_number;
...
void
Check_all_pools ( int max_pools, int *primers_in_pool, PNODE ***primer_pool, int max_amplicon_length )
{
    double max_amp_diff = (double) (max_amplicon_length * 0.15) + 1;
    printf("%s\t%s\t%s\t%s\n", "Pool Number", "Primer Pair Count", "Compatable", "Comparisons");
    for (int i = 0; i <= max_pools; i++ )
    {
        if (primers_in_pool[i] > 1 )
        {
            check_poolability( primer_pool[i], primers_in_pool[i], i, (int) max_amp_diff);
        }
        else
        {
            printf("%d\t1\tYes\t%d\n", (i + 1), 0);
...
void
check_poolability (PNODE ** primer_pool, int primers_in_pool, int pool_number, int max_amp_diff)
{
    ...
    for (i = 0; i < primers_in_pool; i++)
        for (j = i + 1; j < primers_in_pool; j++)
        {
            ...
            cmat[i][j] = is_poolable_primer (primer_pool[i], primer_pool[j], max_amp_diff, 2);
            cmat[j][i] = cmat[i][j];
